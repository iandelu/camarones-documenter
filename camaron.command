#!/bin/sh
# 🦐 Camarón launcher (macOS / Linux). Runs only local files; opens the wizard, or a command: ./camaron.command help
cd "$(dirname "$0")" || exit 1
...
if [ "$(uname)" = "Darwin" ] && command -v xattr >/dev/null 2>&1; then
  xattr -dr com.apple.quarantine .camarones camaron.command camaron.cmd camarones.command camarones.cmd 2>/dev/null
fi
...
if ! command -v uv >/dev/null 2>&1; then
  if command -v brew >/dev/null 2>&1; then
    ...
    echo "🦐 Camarón needs uv (Python tool manager by Astral, brings its own Python)."
    ...
    case "$ans" in
      ...
      *) echo "   Install uv and open Camarón again."; exit 1 ;;
    esac
