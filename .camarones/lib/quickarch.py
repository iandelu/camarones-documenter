...
from .common import ROOT, DOCS, CACHE, run, which
from . import docs
...
def scan() -> dict:
    ...
    ws = docs.workspace()
    names = [r["name"] for r in ws["repos"] if (ROOT / r["name"]).is_dir()]
    ...
    for n in names:
        ...
        blob = "".join(texts)
        blobs[n] = blob
        model["repos"][n] = {"kind": kinds.get(n, "service"), "tech": stack(repo, blob)}
...
Generated by a static scan of the repositories. Open the interactive C4 explorer in the portal (`/architecture/`).
