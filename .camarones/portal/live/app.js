  ...
  $('#main').innerHTML = `<div class="page"><h1>📚 ${esc(t('wikis'))} <span class="muted small">OpenWiki</span></h1>
    ...
    <div class="cards">${rows.map((w) => `<div class="card ${w.repo === open ? 'open' : ''}" data-repo="${esc(w.repo)}">
      ...
      <div class="row">
        ...
        ${!STATIC && engines.length && w.cloned ? `<button class="small w-gen">${esc(w.pages ? t('update') : t('generate'))}</button>${engSel ? ` ${esc(t('via'))} ${engSel}` : ` <span class="muted small">${esc(t('via'))} ${esc(engines[0])}</span>`}` : ''}
      </div>
